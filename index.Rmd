...
income_pal <- colorQuantile(palette = "Purples", n = 5, domain = tract_income$estimate)
income_map <- tract_income %>%
  st_transform("+proj=longlat +datum=WGS84") %>%
  leaflet() %>% 
  addProviderTiles("Esri") %>% 
  addPolygons(color = ~ income_pal(estimate)) %>%
  setView(lng = -78.878738, lat = 42.880230, zoom = 8)
income_map
```
